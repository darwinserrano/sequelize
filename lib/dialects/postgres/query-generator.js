...
const QueryGenerator = {
  ...
  showTablesQuery(schemaName) {
    ...
    return `SELECT table_name FROM information_schema.tables WHERE table_schema = ${this.escape(schemaName)} AND table_type LIKE '%TABLE' AND table_name != 'spatial_ref_sys';`;
  },
   ...
   * @private
   */
  dropForeignKeyQuery(tableName, foreignKey, schemaName) {
    schemaName = schemaName || 'public';
    return `ALTER TABLE ${this.quotedTable(schemaName)}.${this.quoteTable(tableName)} DROP CONSTRAINT ${this.quoteIdentifier(foreignKey)};`;
  },
